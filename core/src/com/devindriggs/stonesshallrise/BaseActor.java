...
public class BaseActor extends Group
{
    ...
    public void addSpeed(float speed, int direction) {
        if (velocityVec.len() == 0) {
            velocityVec.set(speed, direction);
        }
        else {
            velocityVec.add(new Vector2(speed, 0).setAngle(direction));
        }
    }
    ...
    public void accelerateBy(float accel, float angle)
    {
        accelerationVec.add(new Vector2(accel, 0).setAngle(angle));
    }
